...
#define PORT_STRCHR(s, n)			strchr((s), (n))
#define PORT_STRLEN(s)				strlen((s))

#define PORT_ISXDIGIT			isxdigit
#define PORT_ISSPACE(c)		(((c)>=0x09 && (c)<=0x0D) || ((c)==0x20))
#define PORT_ISDIGIT(c)		(((c)>='0') && ((c)<='9'))
#define PORT_ISALPHA(c)		(((c) >='a' && (c) <='z') || ((c) >='A' && (c) <='Z'))
#define PORT_ISUPPER(c)		(((c)>='A') && ((c)<='Z'))
#define PORT_ISLOWER(c)		(((c)>='a') && ((c)<='z'))
#define PORT_TOLOWER(c) 	PORT_ISUPPER(c) ? (c) - 'A' + 'a' : (c)
#define PORT_TOUPPER(c)		PORT_ISLOWER (c) ? (c) - 'a' + 'A' : (c)
...
static int is_metacharacter(const unsigned char *s) {
	...
	return PORT_STRCHR(metacharacters, *s) != NULL;
}
...
static int toi(int x) {
	return PORT_ISDIGIT(x) ? x - '0' : x - 'W';
}

static int hextoi(const unsigned char *s) {
	return (toi(PORT_TOLOWER(s[0])) << 4) | toi(PORT_TOLOWER(s[1]));
}
...
struct regex_info *info) {
	...
	case '\\':
		/* Metacharacters */
		switch (re[1]) {
		case 'S': FAIL_IF(PORT_ISSPACE(*s), SLRE_NO_MATCH); result++; break;
		case 's': FAIL_IF(!PORT_ISSPACE(*s), SLRE_NO_MATCH); result++; break;
		case 'd': FAIL_IF(!PORT_ISDIGIT(*s), SLRE_NO_MATCH); result++; break;
	...
	default:
		if (info->flags & SLRE_IGNORE_CASE) {
			FAIL_IF(PORT_TOLOWER(*re) != PORT_TOLOWER(*s), SLRE_NO_MATCH);
		}
		else {
	...
	while (len <= re_len && re[len] != ']' && result <= 0) {
		...
		if (re[len] != '-' && re[len + 1] == '-' && re[len + 2] != ']' &&
			...
			result = info->flags &&  SLRE_IGNORE_CASE ?
				*s >= re[len] && *s <= re[len + 2] :
				(int)(PORT_TOLOWER((int)*s)) >= (int)(PORT_TOLOWER((int)re[len])) && (int)(PORT_TOLOWER((int)*s)) <= (int)(PORT_TOLOWER((int)re[len + 2]));
			len += 3;
		}
		else {
...
struct regex_info *info, int bi) {
	...
	if (info->recursion_level++ > SLRE_RECURSION_LEVEL)
		return SLRE_RECURSION_ERROR;
...
#define FAIL_IF1(condition, error_code) \
	if (condition) \
		{\
		info->recursion_level--;\
		return (error_code);\
		}

		FAIL_IF1(is_quantifier(&re[i]), SLRE_UNEXPECTED_QUANTIFIER);
		FAIL_IF1(step <= 0, SLRE_INVALID_CHARACTER_SET);
		...
		if (i + step < re_len && is_quantifier(re + i + step)) {
			...
			if (re[i + step] == '?') {
				...
				FAIL_IF1(re[i + step] == '+' && nj == j, SLRE_NO_MATCH);

				/* If while loop body above was not executed for the * quantifier,  */
				/* make sure the rest of the regex matches                          */
				FAIL_IF1(nj == j && ni < re_len && n2 < 0, SLRE_NO_MATCH);
		...
		if (re[i] == '[') {
			...
			FAIL_IF1(n <= 0, SLRE_NO_MATCH);
			j += n;
		}
		else if (re[i] == '(') {
			n = SLRE_NO_MATCH;
			bi++;
			FAIL_IF1(bi >= info->num_brackets, SLRE_INTERNAL_ERROR);
			DBG(("CAPTURING [%.*s] [%.*s] [%s]\n",
			...
			FAIL_IF1(j != 0, SLRE_NO_MATCH);
		}
		else if (re[i] == '$') {
			FAIL_IF1(j != s_len, SLRE_NO_MATCH);
		}
		else {
			FAIL_IF1(j >= s_len, SLRE_NO_MATCH);
...
struct slre_cap *caps, int num_caps, int flags) {
	...
	info.caps = caps;
	info.recursion_level = 0;

	DBG(("========================> [%s] [%.*s]\n", regexp, s_len, s));
	return foo(regexp, (int)PORT_STRLEN(regexp), s, s_len, &info);
}
